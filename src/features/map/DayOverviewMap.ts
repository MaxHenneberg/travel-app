type Stop = { id: string; title: string; location?: string; startsAt?: string; lat?: number; lng?: number };
type Transit = { type: 'transit'; fromStopId: string; toStopId: string };
const valid = (stop: Stop) => Number.isFinite(stop.lat) && Number.isFinite(stop.lng) && Math.abs(stop.lat!) <= 90 && Math.abs(stop.lng!) <= 180;
...
export function mountDayOverviewMap(host: HTMLElement, items: Array<Stop | Transit>, onSelect: (id: string) => void) {
  ...
  const lats = pins.map((s) => s.lat!); const lngs = pins.map((s) => s.lng!);
  const minLat = Math.min(...lats); const maxLat = Math.max(...lats); const minLng = Math.min(...lngs); const maxLng = Math.max(...lngs);
  const project = (stop: Stop) => ({ x: 12 + ((stop.lng! - minLng) / (maxLng - minLng || 1)) * 76, y: 88 - ((stop.lat! - minLat) / (maxLat - minLat || 1)) * 76 });
